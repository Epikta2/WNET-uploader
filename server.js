...
// Bandwidth throttling options - DISABLED FOR MAXIMUM DOWNLOAD SPEED
// const throttleOptions = {
//     bps: 1024 * 1024, // 1MB/s default
//     burst: 1024 * 1024 * 2 // 2MB burst
// };

// Apply bandwidth throttling to download endpoint - DISABLED
...
app.get('/api/files', async (req, res) => {
    try {
        ...
        const files = (response.Contents || [])
            .filter(item => {
                // Filter out test-upload bin files and other test files
                const fileName = item.Key.toLowerCase();
                
                // Exclude files that contain 'test-upload' or 'test_upload'
                if (fileName.includes('test-upload') || fileName.includes('test_upload')) {
                    return false;
                }
                
                // Exclude .bin files that contain 'test' in the name
                if (fileName.endsWith('.bin') && fileName.includes('test')) {
                    return false;
                }
...
// Download endpoint with caching headers and range request support
app.all('/api/download/:filename', async (req, res) => {
    try {
        const { filename } = req.params;
        const range = req.headers.range;
        
        console.log(`Download request for: ${filename}${range ? ` with range: ${range}` : ''}`);
        
        // First, get object metadata to check size and other properties
        ...
        if (range) {
            ...
            // Stream the range to client
            rangeResponse.Body.pipe(res);
            
        } else {
            // Full file download (single stream)
...
// Add bandwidth control endpoint - DISABLED (throttleOptions commented out)
